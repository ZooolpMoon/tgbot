...
> 当前版本：**v2.4.0**（2026-09-12） · 变更见 [CHANGELOG.md](CHANGELOG.md)
...
AI 对话、知识库检索、游戏大厅、每日签到、积分商城、兑换码、每日任务都能开关，分三层设置：
...
| `/ban <用户ID> [原因]` | 把用户加入封禁名单（私聊与所有群停止服务） |
| `/unban <用户ID>` | 把用户移出封禁名单 |
| `/shop_admin` | 商城管理（仅私聊） |
...
npm test                               # 跑测试（内存 SQLite，126 个用例）
