...
> 当前版本：**v2.2.0**（2026-09-12） · 变更见 [CHANGELOG.md](CHANGELOG.md)
...
AI 对话、游戏大厅、每日签到、积分商城、兑换码、每日任务都能开关，分三层设置：
...
`/admin` 面板里还能做：用户管理（👥 一级菜单 → 💬 私聊用户 / 👥 群组用户，列表可翻页）、场景编辑（限额、频率、封禁、清空记忆、功能开关、删除场景）、积分增减与流水、系统状态、使用统计、兑换码、全局功能开关、操作日志、群发入口。
...
- `/admin` —— 主面板（30 分钟会话）
- 面板按钮：👥 用户管理、🛒 商城管理、🎟️ 兑换码、✅ 每日任务、⚙️ 功能开关、📋 操作日志、📊 运行状态、📈 使用统计、🧹 清空我的记忆、❌ 关闭菜单
- 用户管理是**二级菜单**：`👥 用户管理` → `💬 私聊用户` / `👥 群组用户`（列表底部可返回用户管理）
...
以下动作都会写入 `admin_logs`：添加/编辑/上下架/删除商品、订单完成/取消退款、兑换码生成与启停、调整积分、调限额/频率、删除场景、清除记忆、封禁/解封、群发完成/取消。
...
└── src/
    ...
    ├── admin/                   # 管理面板 UI（用户·积分·限额·频率·统计·日志·兑换码·任务·功能开关）
    ├── games/                   # 游戏注册表 + 4 个游戏
...
npm test                               # 跑测试（内存 SQLite，102 个用例）
