...
> 当前版本：**v2.1.0**（2026-09-12） · 变更见 [CHANGELOG.md](CHANGELOG.md)
...
AI 对话、游戏大厅、每日签到、积分商城、兑换码、每日任务都能**全局**开关：

- 位置：`/admin → ⚙️ 功能开关（全局）`，开关对所有私聊与群聊生效
...
| `test/features.test.mjs` | 功能开关（全局）：默认值、开关持久化、恢复默认、设置读写 |
...
| `test/flows.test.mjs` | 入口链路（桩掉 Telegram API，含限购、功能开关、任务引导式增改、定时任务入口） |
...
### Q15. 想让某个群不能玩游戏 / 关掉商城
用**全局功能开关**：`/admin → ⚙️ 功能开关（全局）`，关掉即对所有私聊与群聊生效（v2.1.0 起不再支持单场景覆盖）。
开关是全局设置，清除后恢复默认开启。
